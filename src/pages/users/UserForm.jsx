  ...
  return (
    <div className="min-h-screen bg-gray-50 p-8 flex justify-center items-start pt-16">
      <div className="w-full max-w-lg bg-card p-8 rounded-xl shadow-sm border">
        <h1 className="text-2xl font-bold mb-6 text-foreground">{t('users.addNew')}</h1>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium mb-1">{t('users.temporaryPassword')}</label>
            <input type="password" minLength="6" className="w-full p-2.5 border rounded bg-white focus:ring-2 focus:ring-primary focus:outline-none" required value={formData.password} onChange={e => setFormData({...formData, password: e.target.value})} />
          </div>
          <div>
          ...
          <div className="flex justify-end space-x-3 pt-4 border-t mt-6">
            ...
            <button type="submit" className="px-4 py-2 bg-primary text-primary-foreground rounded text-sm font-medium hover:opacity-90 transition-opacity">{t('users.createEmployee')}</button>
          </div>
        </form>
      </div>
    </div>
