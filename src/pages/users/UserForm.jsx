  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      if (pendingFile) {
        ...
        const {
          data
        } = await axios.post(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/upload/users`, formDataFile, uploadConfig);
        uploadedUrl = data;
      }
      const payload = {
        ...formData,
      ...
      await axios.post(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/auth/register`, payload, config);
      navigate('/users');
    } catch (error) {
      console.error(error);
      setUploading(false);
      alert(error.response?.data?.message || t('users.errorCreating'));
    }
  };
      ...
      <div className="max-w-4xl mx-auto bg-white rounded-[2rem] shadow-sm border border-slate-200/60 overflow-hidden">
        ...
        <div className="p-8 border-b flex justify-between items-center bg-white">
          <div className="flex items-center gap-4">
             ...
             <div>
               <h1 className="text-2xl font-black text-slate-900 tracking-tight uppercase">{t('users.addNew')}</h1>
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div>
              <label className="block text-xs font-black uppercase text-slate-400 tracking-widest mb-2">{t('users.temporaryPassword')}</label>
              <input type="password" minLength="6" required value={formData.password} onChange={e => setFormData({
              ...formData,
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div>
              ...
              <div className="flex items-center gap-4">
                {previewUrl || formData.avatarUrl || formData.firstName ? <img src={previewUrl || (formData.avatarUrl ? formData.avatarUrl.startsWith('/uploads') ? `${import.meta.env.VITE_API_URL || 'http://localhost:5000'}${formData.avatarUrl}` : formData.avatarUrl : `https://ui-avatars.com/api/?name=${formData.firstName}+${formData.lastName}&background=random`)} alt="Preview" className="w-12 h-12 rounded-full object-cover border-2 border-slate-200 shadow-sm" /> : <div className="w-12 h-12 rounded-full bg-slate-50 flex items-center justify-center border-2 border-dashed border-slate-200 text-slate-400">
                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"></path><circle cx="12" cy="7" r="4"></circle></svg>
                  </div>}
                <div className="flex-1">
                  <div className="flex gap-2 items-center">
                    <input id="avatar-upload" type="file" onChange={handleFileSelect} className="w-full text-xs font-bold text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-black file:uppercase file:tracking-widest file:bg-slate-50 file:text-slate-700 hover:file:bg-slate-100 transition-all cursor-pointer" />
                    {(pendingFile || formData.avatarUrl) && <button type="button" onClick={handleRemoveImage} className="px-3 py-2 bg-rose-50 text-rose-600 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-rose-100 transition-colors">
                        {t('common.remove')}
                      </button>}
                  </div>
          ...
          <div className="flex justify-end gap-3 pt-6 border-t border-slate-100">
            <button type="button" onClick={() => navigate('/users')} className="px-6 py-3 border border-slate-200 rounded-2xl text-xs font-black uppercase tracking-widest text-slate-500 hover:bg-slate-50 transition-colors">
              {t('common.cancel')}
            </button>
