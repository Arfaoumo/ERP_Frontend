...
const CustomerList = () => {
  ...
  const filteredCustomers = customers.filter(c => {
    const matchesSearch = c.name.toLowerCase().includes(searchTerm.toLowerCase()) || c.email && c.email.toLowerCase().includes(searchTerm.toLowerCase()) || c.cin && c.cin.toLowerCase().includes(searchTerm.toLowerCase());
          ...
          <div className="p-8 border-b flex flex-col md:flex-row justify-between items-center bg-white gap-4">
            ...
            <div className="flex flex-wrap items-center gap-4">
              ...
              <div className="relative">
                <input type="text" placeholder={t('common.search', 'Search...')} value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="pl-9 pr-4 py-2 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-primary/20 outline-none w-64" />
              ...
              <div className="flex items-center gap-2 px-3 border-l ml-2">
                ...
                <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)} className="bg-gray-50 border border-gray-200 text-gray-700 text-xs rounded-xl focus:ring-blue-500 focus:border-blue-500 block p-2 outline-none font-bold">
              ...
              {}
              <button onClick={() => navigate('/customers/new')} className="px-5 py-2.5 bg-blue-600 text-white rounded-xl text-xs font-black uppercase tracking-widest hover:bg-blue-700 transition-all shadow-lg shadow-blue-200 active:scale-95">
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody>
                {loading ? <tr>
                  ...
                  </tr> : filteredCustomers.map(c => <tr key={c._id} className="border-b hover:bg-gray-50/80 transition-colors">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-center">
                      <div className="flex flex-col justify-center items-center gap-2">
                        <button onClick={() => navigate(`/customers/edit/${c._id}`)} className="text-[10px] font-black text-blue-600 hover:text-blue-800 uppercase tracking-widest transition-all">
                          {t('customers.editProfile')}
                        </button>
                        <button onClick={() => navigate(`/customers/${c._id}`)} className="text-[10px] font-black text-slate-600 hover:text-slate-800 uppercase tracking-widest transition-all">
                ...
                {!loading && filteredCustomers.length === 0 && <tr>
                    <td colSpan="5" className="p-20 text-center">
