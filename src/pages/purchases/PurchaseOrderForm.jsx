...
const PurchaseOrderForm = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const [suppData, prodData, catData] = await Promise.all([axios.get(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/suppliers`, config), axios.get(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/products`, config), axios.get(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/categories`, config)]);
  ...
  const handleOcrResolved = ({
    ...
    if (updatedSupplier && updatedSupplier._id) {
      setSuppliers(prev => prev.map(s => s._id === updatedSupplier._id ? {
        ...s,
  ...
  return <div className="min-h-screen bg-[#f8fafc] p-8">
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-8">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div>
              ...
              <input type="text" value={documentNumber} readOnly className="w-full p-3 border border-slate-200 rounded-xl bg-slate-50 font-mono text-slate-400 font-bold cursor-not-allowed" title={t('purchases.autoGeneratedDoc')} required />
            </div>
            <div>
            ...
            }} className="w-full p-3 bg-white border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500/20 outline-none transition-all text-sm font-bold text-slate-900" required>
          ...
          <div className="space-y-4">
            <div className="flex justify-between items-center border-b border-slate-100 pb-4">
              ...
              {supplier && filteredProductsList.length === 0 && <span className="text-[10px] font-black text-amber-500 uppercase tracking-widest">{t('purchases.supplierNoProducts')}</span>}
            </div>
            {orderItems.map((item, index) => <div key={index} className="grid grid-cols-1 md:grid-cols-12 gap-4 items-end bg-slate-50/50 p-6 rounded-2xl border border-slate-100">
                <div className="md:col-span-5">
                  <label className="block text-[10px] font-black uppercase text-slate-400 mb-1 tracking-widest">{t('common.product')}</label>
                  <select value={item.product} onChange={e => updateItem(index, 'product', e.target.value)} className="w-full p-2.5 bg-white border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500/20 outline-none text-sm font-bold text-slate-900 disabled:opacity-50 transition-all" required disabled={!supplier}>
                ...
                <div className="md:col-span-2">
                  ...
                  <input type="number" value={item.quantity === '' ? '' : item.quantity} onChange={e => updateItem(index, 'quantity', e.target.value === '' ? '' : parseInt(e.target.value))} min="1" className="w-full p-2.5 bg-white border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500/20 outline-none text-sm font-bold text-slate-900 transition-all" required />
                </div>
                <div className="md:col-span-2">
                  <label className="block text-[10px] font-black uppercase text-slate-400 mb-1 tracking-widest">{t('purchases.buyingPrice')}</label>
                  <input type="number" value={item.buyingPrice === '' ? '' : item.buyingPrice} readOnly step="0.01" className="w-full p-2.5 border border-slate-200 rounded-xl bg-slate-50 font-mono text-slate-400 font-bold cursor-not-allowed" required title={t('purchases.buyingPriceTooltip')} />
                </div>
            ...
            <div className="flex justify-between items-center flex-wrap gap-3 pt-2">
              <button type="button" onClick={addItem} className="text-[10px] font-black text-blue-600 hover:text-blue-800 uppercase tracking-widest flex items-center gap-1 transition-all">
                {t('purchases.addAnotherProduct')}
              </button>
              <div className="flex items-center gap-4">
                <button type="button" onClick={() => setOcrModalOpen(true)} disabled={!supplier} title={!supplier ? t('purchases.ocr.selectSupplierFirst') : ''} className="text-[10px] font-black text-indigo-600 hover:text-indigo-800 uppercase tracking-widest flex items-center gap-1.5 disabled:text-slate-400 disabled:cursor-not-allowed transition-all">
                ...
                <button type="button" onClick={() => setProductModalOpen(true)} className="text-[10px] font-black text-emerald-600 hover:text-emerald-800 uppercase tracking-widest flex items-center gap-1.5 transition-all">
      ...
      {}
      <OcrInvoiceModal isOpen={ocrModalOpen} onClose={() => setOcrModalOpen(false)} supplier={supplier} supplierName={suppliers.find(s => s._id === supplier)?.name} categories={categories} onResolved={handleOcrResolved} />
          ...
          <div className="w-full max-w-lg bg-white rounded-2xl shadow-2xl border border-slate-100 overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            ...
            <form onSubmit={handleCreateProduct} className="p-6 space-y-4 max-h-[75vh] overflow-y-auto">
              ...
              <div>
                ...
                <select className="w-full p-2.5 border rounded-lg bg-white focus:ring-2 focus:ring-primary focus:outline-none text-sm" required value={newProductData.category} onChange={e => setNewProductData({
              ...newProductData,
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="number" step="0.01" className="w-full p-2.5 border rounded-lg bg-white focus:ring-2 focus:ring-primary focus:outline-none text-sm" required value={newProductData.sellingPrice === '' ? '' : newProductData.sellingPrice} onChange={e => setNewProductData({
                ...newProductData,
                ...
                <div>
                  ...
                  <input type="number" step="0.01" className="w-full p-2.5 border rounded-lg bg-white focus:ring-2 focus:ring-primary focus:outline-none text-sm" required value={newProductData.buyingPrice === '' ? '' : newProductData.buyingPrice} onChange={e => setNewProductData({
                ...newProductData,
                buyingPrice: e.target.value === '' ? '' : parseFloat(e.target.value)
              })} />
                </div>
              </div>
              ...
              <div>
                ...
                <input type="number" className="w-full p-2.5 border rounded-lg bg-white focus:ring-2 focus:ring-primary focus:outline-none text-sm" required value={newProductData.minStockThreshold === '' ? '' : newProductData.minStockThreshold} onChange={e => setNewProductData({
              ...newProductData,
              minStockThreshold: e.target.value === '' ? '' : parseInt(e.target.value)
            })} />
              </div>

              {supplier ? <div className="p-3.5 bg-emerald-50 border border-emerald-200 text-emerald-800 text-xs rounded-xl flex items-center gap-2">
                  <span className="relative flex h-2 w-2">
                    <span className="relative inline-flex rounded-full h-2 w-2 bg-emerald-500"></span>
                  </span>
                  <span dangerouslySetInnerHTML={{
              __html: t('purchases.willBeLinked', {
                name: suppliers.find(s => s._id === supplier)?.name
              })
            }}></span>
                </div> : <div className="p-3.5 bg-amber-50 border border-amber-200 text-amber-800 text-xs rounded-xl flex items-center gap-2">
                  <span className="relative flex h-2 w-2">
