...
const PurchaseOrderList = () => {
  ...
  const fetchOrders = async () => {
    try {
      ...
      const {
        data
      } = await axios.get(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/purchases/orders`, config);
      setOrders(data);
    } catch (error) {
  ...
  const filteredOrders = orders.filter(o => o.documentNumber?.toLowerCase().includes(searchTerm.toLowerCase()) || o.supplier?.name?.toLowerCase().includes(searchTerm.toLowerCase()));
  const confirmStatusUpdate = async () => {
    ...
    try {
      ...
      await axios.put(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/purchases/orders/${id}/status`, {
        status
      }, config);
      fetchOrders();
    } catch (error) {
  ...
  const openStatusModal = (id, status) => {
    ...
    setModalConfig({
      ...
      message: isReceive ? t('purchases.receiveStockModalMsg') : t('purchases.cancelOrderModalMsg'),
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        <div className="bg-white rounded-[2rem] shadow-sm border border-slate-200/60 overflow-hidden">
          ...
          <div className="flex items-center gap-4 w-full md:w-auto">
            ...
            <button onClick={() => navigate('/purchases/orders/new')} className="bg-blue-600 text-white px-6 py-2.5 rounded-2xl text-sm font-black uppercase tracking-widest hover:bg-blue-700 transition-all shadow-lg shadow-blue-200 active:scale-95 whitespace-nowrap">
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody>
                {loading ? <tr>
                  ...
                  </tr> : filteredOrders.map(order => <tr key={order._id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors group">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-center">
                      <div className="flex items-center justify-center gap-3">
                        <button onClick={() => openDetailsModal(order)} className="text-[10px] font-black text-slate-500 hover:text-slate-700 uppercase tracking-widest transition-all">
                          {t('purchases.viewDetails')}
                        </button>
                        {order.status === 'Pending' && <>
                            <button onClick={() => openStatusModal(order._id, 'Received')} className="text-[10px] font-black text-emerald-600 hover:text-emerald-800 uppercase tracking-widest transition-all">
                              {t('purchases.receiveStock')}
                            </button>
                            <button onClick={() => openStatusModal(order._id, 'Cancelled')} className="text-[10px] font-black text-red-500 hover:text-red-700 uppercase tracking-widest transition-all">
                              {t('purchases.cancel')}
                            </button>
                          </>}
                      </div>
                ...
                {!loading && filteredOrders.length === 0 && <tr>
        ...
        <ConfirmationModal isOpen={modalOpen} onClose={() => setModalOpen(false)} onConfirm={confirmStatusUpdate} title={modalConfig.title} message={modalConfig.message} confirmText={modalConfig.status === 'Received' ? t('purchases.receiveGoodsBtn') : t('purchases.yesCancelBtn')} type={modalConfig.type} />

        {detailsModalOpen && selectedOrder && <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
            <div className="w-full max-w-2xl bg-white rounded-[2rem] shadow-2xl border border-slate-100 overflow-hidden animate-in fade-in zoom-in-95 duration-200">
              <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50">
                ...
                <button onClick={() => setDetailsModalOpen(false)} className="text-slate-400 hover:text-slate-600 font-mono text-xl">
                  &times;
                </button>
              </div>
              <div className="p-6 max-h-[70vh] overflow-y-auto">
                ...
                <div className="space-y-3">
                  {(selectedOrder.products || []).map((item, index) => <div key={index} className="flex justify-between items-center bg-slate-50 p-4 rounded-xl border border-slate-100">
                      <div>
              ...
              <div className="p-6 border-t border-slate-100 bg-slate-50 flex justify-end">
                <button onClick={() => setDetailsModalOpen(false)} className="px-6 py-2.5 border border-slate-200 rounded-2xl text-xs font-black uppercase tracking-widest text-slate-500 hover:bg-white transition-colors shadow-sm">
                  {t('common.close')}
                </button>
              </div>
            </div>
