...
const OcrInvoiceModal = ({ isOpen, onClose, supplier, supplierName, categories, onResolved }) => {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
      <div className="w-full max-w-5xl bg-white rounded-2xl shadow-2xl border border-slate-100 overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="p-6 overflow-y-auto flex-1 space-y-5">
          {!supplier && (
            <div className="p-3.5 bg-amber-50 border border-amber-200 text-amber-800 text-xs rounded-xl">
          ...
          {supplier && !extracted && (
            <div className="space-y-4">
              <div
                className="p-3.5 bg-emerald-50 border border-emerald-200 text-emerald-800 text-xs rounded-xl"
                dangerouslySetInnerHTML={{ __html: t('purchases.ocr.targetSupplier', { name: supplierName || '' }) }}
              ></div>
              <div>
              ...
              {previewUrl && (
                <div className="border rounded-lg p-3 bg-slate-50 flex justify-center">
                  <img src={previewUrl} alt="invoice preview" className="max-h-80 rounded object-contain" />
                </div>
              )}
              {file && !previewUrl && (
                <div className="border rounded-lg p-3 bg-slate-50 text-sm text-slate-600 flex items-center gap-2">
          ...
          {extracted && (
            <div className="space-y-4">
              <div className="flex justify-between items-center">
                ...
                <button
                  type="button"
                  onClick={() => { setExtracted(null); setRows([]); setError(''); }}
              ...
              {(extracted.documentNumber || extracted.supplierName || extracted.totalAmount) && (
                <div className="text-xs text-slate-600 grid grid-cols-1 md:grid-cols-3 gap-2 bg-slate-50 p-3 rounded-lg border">
                  {extracted.supplierName && (
                    <div><span className="font-bold text-slate-500 uppercase tracking-wider">{t('purchases.ocr.detectedSupplier')}: </span>{extracted.supplierName}</div>
                  )}
                  {extracted.documentNumber && (
                    <div><span className="font-bold text-slate-500 uppercase tracking-wider">{t('purchases.ocr.detectedDocNumber')}: </span><span className="font-mono">{extracted.documentNumber}</span></div>
                  )}
                  {extracted.totalAmount !== null && extracted.totalAmount !== undefined && (
                    <div><span className="font-bold text-slate-500 uppercase tracking-wider">{t('purchases.ocr.detectedTotal')}: </span>€{Number(extracted.totalAmount).toFixed(2)}</div>
                  )}
                </div>
              )}
              ...
              <div className="overflow-x-auto border rounded-lg">
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {rows.map((row, idx) => (
                      <tr key={idx} className={`border-b align-top ${row.useExisting ? 'bg-emerald-50/40' : 'bg-amber-50/30'}`}>
                        <td className="p-2 align-middle">
                          {row.matchedProductId ? (
                            <div className="space-y-1">
                              ...
                              <button
                                type="button"
                                onClick={() => updateRow(idx, { useExisting: !row.useExisting })}
                            ...
                            <span className="inline-block text-[9px] font-bold uppercase px-2 py-0.5 rounded bg-indigo-100 text-indigo-700">
                        ...
                        <td className="p-2">
                          <input
                            ...
                            disabled={row.useExisting}
                            className="w-full p-1.5 border rounded text-sm disabled:bg-slate-100 disabled:text-slate-500"
                          />
                          {row.useExisting && row.matchedProductName && row.matchedProductName !== row.name && (
                            <p className="text-[10px] text-slate-400 mt-1">{t('purchases.ocr.dbName')}: {row.matchedProductName}</p>
                          )}
                        </td>
                        <td className="p-2">
                          <input
                            ...
                            disabled={row.useExisting}
                            placeholder={row.useExisting ? '' : t('purchases.skuPlaceholder')}
                            className="w-full p-1.5 border rounded text-sm font-mono disabled:bg-slate-100 disabled:text-slate-500"
                          />
                        </td>
                        <td className="p-2">
                          {row.useExisting ? (
                            <span className="text-xs text-slate-600">{row.matchedCategoryName || '—'}</span>
                          ) : (
                            <select
                    ...
                    {rows.length === 0 && (
                      <tr><td colSpan="7" className="p-6 text-center text-slate-400 text-sm">{t('purchases.ocr.noRows')}</td></tr>
                    )}
                  </tbody>
                  {rows.length > 0 && (
                    <tfoot>
        ...
        <div className="p-4 border-t border-slate-100 bg-slate-50 flex justify-end gap-3">
          <button
            type="button"
            onClick={handleClose}
            className="px-4 py-2 border rounded-lg text-sm font-medium hover:bg-white transition-colors"
          >
            {t('common.cancel')}
          </button>
          {!extracted ? (
            <button
              ...
              disabled={!supplier || !file || extracting}
              className="px-5 py-2 bg-indigo-600 text-white rounded-lg text-sm font-bold hover:bg-indigo-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex items-center gap-2"
            >
              {extracting && (
                <svg className="animate-spin" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3"><circle cx="12" cy="12" r="10" opacity="0.25"></circle><path d="M22 12a10 10 0 0 1-10 10"></path></svg>
              )}
            ...
            <button
              ...
              className="px-5 py-2 bg-emerald-600 text-white rounded-lg text-sm font-bold hover:bg-emerald-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex items-center gap-2"
            >
              {resolving && (
                <svg className="animate-spin" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3"><circle cx="12" cy="12" r="10" opacity="0.25"></circle><path d="M22 12a10 10 0 0 1-10 10"></path></svg>
              )}
              {resolving ? t('purchases.ocr.importing') : t('purchases.ocr.importBtn', { count: rows.length })}
            </button>
          )}
        </div>
      </div>
