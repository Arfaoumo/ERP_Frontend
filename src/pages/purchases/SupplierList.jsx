...
const SupplierList = () => {
  ...
  const fetchSuppliers = async () => {
    try {
      ...
      const { data } = await axios.get(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/suppliers`, config);
      setSuppliers(data);
    } catch (error) {
            ...
            <tbody>
              {loading ? (
                ...
                <tr key={s._id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors group">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-center">
                    {['Admin', 'Employee_Achats'].includes(user?.role) && (
                      <button onClick={() => navigate(`/purchases/suppliers/edit/${s._id}`)} className="text-[10px] font-black text-blue-600 hover:text-blue-800 uppercase tracking-widest transition-all">{t('common.edit')}</button>
                    )}
                  </td>
                </tr>
              ))}
