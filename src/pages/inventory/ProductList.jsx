...
const ProductList = () => {
  ...
  const fetchProducts = async () => {
    try {
      ...
      const { data } = await axios.get(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/products`, config);
      setProducts(data);
    } catch (error) {
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-white p-6 rounded-2xl border border-slate-200/60 shadow-sm flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-black text-slate-900 mt-1 tracking-tight">€{totalValue.toLocaleString(undefined, { minimumFractionDigits: 2 })}</p>
            </div>
            ...
            <div className="flex flex-wrap items-center justify-end gap-4 w-full xl:w-auto">
              ...
              <div className="flex items-center gap-2 bg-slate-50 px-3 py-2 rounded-xl border border-slate-100">
                ...
                <button
                  ...
                  className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors ${showLowStockOnly ? 'bg-rose-500' : 'bg-slate-300'}`}
                >
              ...
              <div className="flex gap-3 w-full sm:w-auto">
                <button
                  ...
                  className="flex-1 sm:flex-none bg-slate-50 text-slate-600 border border-slate-200 px-6 py-2.5 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-slate-100 hover:text-slate-800 transition-colors shadow-sm"
                >
                ...
                {['Admin', 'Employee_Stocks'].includes(user?.role) && (
                  <button
                    ...
                    className="flex-1 sm:flex-none bg-blue-600 text-white px-6 py-2.5 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-blue-700 transition-all shadow-lg shadow-blue-200 active:scale-95"
                  >
                  ...
                  <tr key={p._id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors group">
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium flex items-center justify-start gap-4">
                      {p.imageUrl ? (
                        ...
                        <div className="w-12 h-12 rounded-xl bg-slate-100 flex items-center justify-center text-slate-300 border border-slate-200 shadow-sm">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-center">
                      {p.currentStock < p.minStockThreshold ? (
                        <div className="flex flex-col items-center">
                          ...
                          <span className="text-[10px] text-rose-400 font-bold mt-1.5 uppercase tracking-widest">{t('inventory.belowMin', {threshold: p.minStockThreshold})}</span>
                        </div>
                      ) : (
                        <span className="inline-flex items-center px-3 py-1 rounded-full text-[10px] font-black tracking-widest bg-emerald-50 text-emerald-600 border border-emerald-100">
                          {p.currentStock} {t('inventory.units')}
                        </span>
                      )}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-center">
                      {['Admin', 'Employee_Stocks'].includes(user?.role) && (
                        <div className="flex flex-col items-center justify-center gap-1.5">
                          {p.isActive !== false ? (
                            <button onClick={() => navigate(`/inventory/stock/${p._id}`)} className="text-[10px] font-black text-blue-600 hover:text-blue-800 uppercase tracking-widest transition-all">
