...
const ProductEditForm = () => {
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        ...
        const {
          data
        } = await axios.get(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/categories`, config);
        setCategories(data);
      } catch (error) {
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      await axios.put(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/products/${id}`, payload, config);
      navigate('/inventory');
    } catch (error) {
      console.error(error);
      setUploading(false);
          ...
          <div className="flex items-center gap-4">
           ...
           <div>
             ...
             <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">
               {t('inventory.updateProduct', 'Mettre à jour le produit')}
             </p>
           </div>
        </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-8">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div>
              ...
              <input className="w-full p-3 bg-white border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500/20 outline-none transition-all text-sm font-bold text-slate-900 uppercase placeholder:text-slate-300" required value={formData.sku} onChange={e => setFormData({
              ...formData,
              sku: e.target.value
            })} placeholder={t('inventory.skuLabel')} />
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div>
              ...
              <select className="w-full p-3 bg-white border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500/20 outline-none transition-all text-sm font-bold text-slate-900" required value={formData.category} onChange={e => setFormData({
              ...formData,
              category: e.target.value
            })}>
                <option value="" disabled>{t('inventory.selectCategory')}</option>
                {categories.map(cat => <option key={cat._id} value={cat._id}>{cat.name} - {(cat.taxRate * 100).toFixed(0)}%</option>)}
              </select>
            </div>
            <div>
              ...
              <input type="number" className="w-full p-3 bg-white border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500/20 outline-none transition-all text-sm font-bold text-slate-900" required value={formData.minStockThreshold} onChange={e => setFormData({
              ...formData,
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div>
              ...
              <input type="number" step="0.01" className="w-full p-3 bg-white border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500/20 outline-none transition-all text-sm font-bold text-slate-900" required value={formData.sellingPrice} onChange={e => setFormData({
              ...formData,
            ...
            <div>
              ...
              <input type="number" step="0.01" className="w-full p-3 bg-white border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500/20 outline-none transition-all text-sm font-bold text-slate-900" required value={formData.buyingPrice} onChange={e => setFormData({
              ...formData,
          ...
          <div>
            ...
            <div className="flex items-center gap-4">
              {previewUrl || formData.imageUrl ? <img src={previewUrl || (formData.imageUrl.startsWith('/uploads') ? `${import.meta.env.VITE_API_URL || 'http://localhost:5000'}${formData.imageUrl}` : formData.imageUrl)} alt="Preview" className="w-16 h-16 rounded-xl object-cover border border-slate-200 shadow-sm" /> : <div className="w-16 h-16 rounded-xl bg-slate-50 flex items-center justify-center border border-dashed border-slate-300 text-slate-400">
              ...
              <div className="flex-1">
                <div className="flex gap-2 items-center">
                  ...
                  {(pendingFile || formData.imageUrl) && <button type="button" onClick={handleRemoveImage} className="px-4 py-2.5 bg-rose-50 text-rose-600 border border-rose-100 rounded-xl hover:bg-rose-100 transition-colors text-xs font-black uppercase tracking-widest">
                      {t('common.remove')}
                    </button>}
                </div>
          ...
          <div className="flex justify-end gap-3 pt-6 border-t border-slate-100">
            ...
            <button type="submit" className="px-8 py-3 bg-blue-600 text-white rounded-2xl text-xs font-black uppercase tracking-widest hover:bg-blue-700 transition-all shadow-lg shadow-blue-200 active:scale-95">{t('inventory.updateProduct')}</button>
          </div>
        </form>
      </div>
    </div>;
