  ...
  return (
    <div className="min-h-screen bg-gray-50 p-8 flex justify-center items-start pt-20">
      <div className="w-full max-w-lg bg-card p-8 rounded-xl shadow-sm border">
        <h1 className="text-2xl font-bold mb-6 text-foreground">{t('inventory.createProduct')}</h1>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <div className="flex items-center gap-4">
              {previewUrl && (
                <img src={previewUrl} alt="Preview" className="w-16 h-16 rounded object-cover border-2 border-gray-200 shadow-sm" />
              )}
          ...
          <div className="flex justify-end space-x-3 pt-4 border-t mt-6">
            ...
            <button type="submit" className="px-4 py-2 bg-primary text-primary-foreground rounded text-sm font-medium hover:opacity-90 transition-opacity">{t('inventory.saveProduct')}</button>
          </div>
        </form>
      </div>
    </div>
