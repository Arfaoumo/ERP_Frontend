        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-8">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div>
              ...
              ...formData,
              name: e.target.value
            })} placeholder={t('inventory.companyNamePlaceholder')} />
            </div>
            <div>
              <label className="block text-xs font-black uppercase text-slate-400 tracking-widest mb-2">{t('auth.emailLabel')}</label>
              <input type="email" className="w-full p-3 bg-white border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500/20 outline-none transition-all text-sm font-bold text-slate-900 lowercase placeholder:text-slate-300 placeholder:uppercase" required value={formData.contactEmail} onChange={e => setFormData({
              ...formData,
              contactEmail: e.target.value
            })} placeholder={t('inventory.emailPlaceholderUpper')} />
            </div>
          </div>

          {isEditMode && <div className="flex items-center p-6 bg-slate-50 rounded-2xl border border-slate-100">
          ...
          <div className="flex justify-end gap-3 pt-6 border-t border-slate-100">
            <button type="button" onClick={() => navigate('/delivery-companies')} className="px-6 py-3 border border-slate-200 rounded-2xl text-xs font-black uppercase tracking-widest text-slate-500 hover:bg-slate-50 transition-colors">
              {t('common.cancel')}
            </button>
