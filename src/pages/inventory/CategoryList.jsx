...
const CategoryList = () => {
  ...
  const filteredCategories = categories.filter(cat => cat.name.toLowerCase().includes(searchTerm.toLowerCase()) || cat.description && cat.description.toLowerCase().includes(searchTerm.toLowerCase()));
          ...
          <div className="p-8 border-b flex flex-col md:flex-row justify-between items-center bg-white gap-4">
            ...
            <div className="flex flex-wrap items-center gap-4 mt-4 md:mt-0">
              ...
              <div className="relative">
                <input type="text" placeholder={t('common.search', 'Search categories...')} value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="pl-9 pr-4 py-2 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-blue-500/20 outline-none w-64 bg-slate-50 font-bold text-slate-900" />
              ...
              <button onClick={() => navigate('/categories/new')} className="bg-blue-600 text-white px-6 py-2.5 rounded-2xl text-sm font-black uppercase tracking-widest hover:bg-blue-700 transition-all shadow-lg shadow-blue-200 active:scale-95">
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody>
                {loading ? <tr>
                  ...
                  </tr> : filteredCategories.map(cat => <tr key={cat._id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors group">
                    ...
                    <td className="px-8 py-6 text-center">
                      <button onClick={() => navigate(`/categories/edit/${cat._id}`)} className="text-[10px] font-black text-blue-600 hover:text-blue-800 uppercase tracking-widest transition-all">
                        {t('inventory.editCategory')}
                      </button>
                    </td>
                  </tr>)}
                {!loading && filteredCategories.length === 0 && <tr>
                    <td colSpan="4" className="p-20 text-center">
