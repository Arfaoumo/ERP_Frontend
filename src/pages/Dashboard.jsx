...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        const [salesRes, alertsRes, metricsRes] = await Promise.all([
          axios.get(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/sales`, config).catch(() => ({ data: [] })),
          axios.get(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/alerts`, config).catch(() => ({ data: [] })),
          axios.get(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/dashboard/metrics`, config).catch(() => ({ data: null }))
        ]);
  ...
  // Map backend metrics to variables for the UI
  ...
  for (let i = 5; i >= 0; i--) {
    ...
    monthlyData[`${d.toLocaleString(i18n.language || 'en-US', { month: 'short' })}`] = 0;
  }
  activeInvoices.forEach(inv => {
    const month = new Date(inv.createdAt).toLocaleString(i18n.language || 'en-US', { month: 'short' });
      ...
      <div className="max-w-7xl mx-auto space-y-8">
            ...
            <div className="hidden md:flex flex-col items-end">
              ...
              <span className="text-[10px] font-black bg-indigo-50 text-indigo-600 px-2 py-0.5 rounded-md uppercase tracking-wider">
                {user?.role === 'Admin' ? t('roles.administrator') : 
                 ...
                 user?.role === 'Employee_Achats' ? t('roles.purchasingManager') : 
                 user?.role === 'Employee_RH' ? 'HR Manager' : user?.role}
              </span>
            </div>
            <button
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <div className="bg-white p-6 rounded-[2rem] border border-slate-100/80 shadow-md shadow-slate-100/40 relative overflow-hidden group hover:shadow-xl hover:shadow-amber-500/5 hover:-translate-y-1 transition-all duration-300">
            ...
            <div className="mt-4 flex items-center gap-2">
              ...
              <span className="text-[10px] font-extrabold text-amber-600 bg-amber-50 px-2 py-0.5 rounded uppercase tracking-wider">{t('dashboard.pendingOrdersCount', { count: pendingOrdersCount })}</span>
            </div>
          </div>
          ...
          <div className="bg-white p-6 rounded-[2rem] border border-slate-100/80 shadow-md shadow-slate-100/40 relative overflow-hidden group hover:shadow-xl hover:shadow-blue-500/5 hover:-translate-y-1 transition-all duration-300">
            ...
            <div className="mt-4 flex items-center gap-2">
              ...
              <span className="text-[10px] font-extrabold text-blue-600 bg-blue-50 px-2 py-0.5 rounded uppercase tracking-wider">{t('dashboard.activeShipments', { count: deliveriesInTransitCount })}</span>
            </div>
          </div>
          ...
          <div className="bg-white p-6 rounded-[2rem] border border-slate-100/80 shadow-md shadow-slate-100/40 relative overflow-hidden group hover:shadow-xl hover:shadow-rose-500/5 hover:-translate-y-1 transition-all duration-300">
            ...
            <div className="mt-4 flex items-center gap-2">
              ...
              <span className="text-[10px] font-extrabold text-rose-600 bg-rose-50 px-2 py-0.5 rounded uppercase tracking-wider">{t('dashboard.overdueInvoices', { count: overdueInvoicesCount })}</span>
            </div>
          </div>
          ...
          <div className="bg-gradient-to-br from-slate-900 via-slate-800 to-indigo-950 p-6 rounded-[2rem] shadow-xl shadow-indigo-950/20 relative overflow-hidden group flex flex-col justify-between border border-slate-800 hover:-translate-y-1 hover:shadow-2xl hover:shadow-indigo-950/30 transition-all duration-300">
            ...
            <div className="mt-4 flex items-center gap-2">
              ...
              <span className="text-[10px] font-extrabold text-emerald-400 bg-emerald-950/50 px-2 py-0.5 rounded uppercase tracking-wider">{t('dashboard.finalizedInvoices', { count: currentMonthPaidInvoicesCount })}</span>
            </div>
          </div>
        </div>

        {/* Secondary Intelligence Row (KPIs) - Added from Spec */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {/* Card 5: Montant des Achats */}
          <div className="bg-white p-6 rounded-[2rem] border border-slate-100/80 shadow-md shadow-slate-100/40 relative overflow-hidden group hover:shadow-xl hover:shadow-indigo-500/5 hover:-translate-y-1 transition-all duration-300">
            ...
            <p className="text-2xl font-black bg-gradient-to-r from-slate-950 to-slate-800 bg-clip-text text-transparent mt-2">€{metrics?.purchasesThisMonth?.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 }) || '0.00'}</p>
            <div className="mt-4 flex items-center gap-2">
              <span className="relative flex h-2 w-2">
                <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-indigo-400 opacity-75"></span>
                <span className="relative inline-flex rounded-full h-2 w-2 bg-indigo-500"></span>
              </span>
              <span className="text-[10px] font-extrabold text-indigo-600 bg-indigo-50 px-2 py-0.5 rounded uppercase tracking-wider">{t('dashboard.ordersThisMonth', { count: metrics?.purchasesCount || 0 })}</span>
            </div>
          </div>

          {/* Card 6: Valeur du Stock */}
          <div className="bg-white p-6 rounded-[2rem] border border-slate-100/80 shadow-md shadow-slate-100/40 relative overflow-hidden group hover:shadow-xl hover:shadow-emerald-500/5 hover:-translate-y-1 transition-all duration-300">
            ...
            <p className="text-2xl font-black bg-gradient-to-r from-slate-950 to-slate-800 bg-clip-text text-transparent mt-2">€{metrics?.totalStockValue?.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 }) || '0.00'}</p>
            <div className="mt-4 flex items-center gap-2">
              <span className="relative flex h-2 w-2">
                <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75"></span>
        ...
        <div className="bg-white p-8 rounded-[2rem] border border-slate-100/80 shadow-md shadow-slate-100/30 h-auto w-full">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 w-full">
            ...
            {['Admin', 'Employee_Commercial', 'Employee_Achats'].includes(user?.role) && (
              <div onClick={() => navigate('/reports')} className="p-6 bg-white border border-slate-100/80 rounded-2xl hover:bg-rose-50/20 hover:border-rose-100 hover:shadow-md hover:shadow-rose-500/5 transition-all duration-300 cursor-pointer group">
            ...
            {/* System Logs (Visible to all users, but filtered by role in backend) */}
                ...
                {alerts.length > 0 ? alerts.map(alert => {
                  ...
                  return (
                    <div 
                      ...
                      className={`p-4 bg-${alertColor}-50/30 border border-${alertColor}-100/50 rounded-2xl group hover:bg-${alertColor}-50/70 hover:border-${alertColor}-200 transition-all duration-300 cursor-pointer`}
                    >
                      <div className="mb-2">
                        <span className={`text-[10px] font-black text-${alertColor}-600 font-mono tracking-tighter bg-${alertColor}-50 px-2 py-0.5 rounded`}>{t(`alerts.title.${alert.type}`, { defaultValue: alert.title })}</span>
                      </div>
                      <p className="text-xs font-black text-slate-800 leading-relaxed">{t(`alerts.message.${alert.type}`, { ...alert.data, defaultValue: alert.message })}</p>
                    </div>
                  );
                }) : (
