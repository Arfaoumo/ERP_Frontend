...
const Dashboard = () => {
  ...
  const overdueInvoices = invoices.filter(s => s.paymentStatus === 'Overdue');
  const pendingInvoices = invoices.filter(s => s.paymentStatus === 'Pending' || s.paymentStatus === 'Partially Paid');
  const paidInvoices = invoices.filter(s => s.paymentStatus === 'Paid');
  
  // Pending Orders calculations
  const pendingOrders = sales.filter(s => s.documentType === 'Order' && s.status === 'Pending');
  const pendingOrdersRevenue = pendingOrders.reduce((acc, s) => acc + (s.totalWithTax || s.totalAmount), 0);
  
  // Deliveries In Transit calculations
  const deliveriesInTransit = sales.filter(s => s.documentType === 'DeliveryNote' && s.status === 'In Transit');
  const transitRevenue = deliveriesInTransit.reduce((acc, s) => acc + (s.totalWithTax || s.totalAmount), 0);
  
  // Overdue Receivables total
  const overdueRevenue = overdueInvoices.reduce((acc, s) => acc + (s.remainingBalance !== undefined ? s.remainingBalance : (s.totalWithTax || s.totalAmount)), 0);

  const currentMonth = new Date().getMonth();
  const currentYear = new Date().getFullYear();
  // We use the date of the last payment (or updatedAt) to determine when it was finalized
  const currentMonthPaidInvoices = paidInvoices.filter(s => {
    const finalDate = (s.payments && s.payments.length > 0) ? new Date(s.payments[s.payments.length - 1].date) : new Date(s.updatedAt);
    return finalDate.getMonth() === currentMonth && finalDate.getFullYear() === currentYear;
  });

  const totalRevenue = currentMonthPaidInvoices.reduce((acc, s) => acc + (s.totalWithTax || s.totalAmount), 0);
  ...
  paidInvoices.forEach(inv => {
    ...
    if (monthlyData[month] !== undefined) monthlyData[month] += inv.totalAmount;
  });
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <header className="flex justify-between items-center p-6 rounded-[2rem] border border-white/50 backdrop-blur-md bg-white/70 sticky top-4 z-50 shadow-lg shadow-slate-100/40 transition-all duration-300">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-xl font-black tracking-tight uppercase text-slate-800">Antigravity<span className="bg-gradient-to-r from-indigo-600 to-violet-600 bg-clip-text text-transparent">ERP</span></h1>
          ...
          <div className="flex items-center gap-6">
            <div className="hidden md:flex flex-col items-end">
              ...
              <span className="text-[10px] font-black bg-indigo-50 text-indigo-600 px-2 py-0.5 rounded-md uppercase tracking-wider">
                {user?.role === 'Admin' ? t('roles.administrator') : 
                 user?.role === 'Employee_Commercial' ? t('roles.commercialOfficer') : 
                 user?.role === 'Employee_Stocks' ? t('roles.inventoryManager') : 
                 user?.role === 'Employee_Achats' ? t('roles.purchasingManager') : user?.role}
              </span>
            </div>
            <button
              onClick={handleLogout}
              className="px-5 py-2.5 bg-slate-900 text-white rounded-xl text-xs font-black uppercase tracking-widest hover:bg-slate-800 hover:shadow-lg hover:shadow-slate-900/20 hover:-translate-y-0.5 transition-all duration-200 active:scale-95"
            >
              {t('dashboard.logout')}
            </button>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <div className="bg-white p-6 rounded-[2rem] border border-slate-100/80 shadow-md shadow-slate-100/40 relative overflow-hidden group hover:shadow-xl hover:shadow-amber-500/5 hover:-translate-y-1 transition-all duration-300">
            ...
            <p className="text-2xl font-black bg-gradient-to-r from-slate-950 to-slate-800 bg-clip-text text-transparent mt-2">€{pendingOrdersRevenue.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
            <div className="mt-4 flex items-center gap-2">
              <span className="relative flex h-2 w-2">
                <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-amber-400 opacity-75"></span>
                <span className="relative inline-flex rounded-full h-2 w-2 bg-amber-500"></span>
              </span>
              <span className="text-[10px] font-extrabold text-amber-600 bg-amber-50 px-2 py-0.5 rounded uppercase tracking-wider">{t('dashboard.pendingOrdersCount', { count: pendingOrders.length })}</span>
            </div>
          </div>

          {/* Card 2: Deliveries In Transit */}
          <div className="bg-white p-6 rounded-[2rem] border border-slate-100/80 shadow-md shadow-slate-100/40 relative overflow-hidden group hover:shadow-xl hover:shadow-blue-500/5 hover:-translate-y-1 transition-all duration-300">
            ...
            <p className="text-2xl font-black bg-gradient-to-r from-slate-950 to-slate-800 bg-clip-text text-transparent mt-2">€{transitRevenue.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
            <div className="mt-4 flex items-center gap-2">
              <span className="relative flex h-2 w-2">
                <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-blue-400 opacity-75"></span>
                <span className="relative inline-flex rounded-full h-2 w-2 bg-blue-500"></span>
              </span>
              <span className="text-[10px] font-extrabold text-blue-600 bg-blue-50 px-2 py-0.5 rounded uppercase tracking-wider">{t('dashboard.activeShipments', { count: deliveriesInTransit.length })}</span>
            </div>
          </div>
          ...
          <div className="bg-white p-6 rounded-[2rem] border border-slate-100/80 shadow-md shadow-slate-100/40 relative overflow-hidden group hover:shadow-xl hover:shadow-rose-500/5 hover:-translate-y-1 transition-all duration-300">
            ...
            <p className="text-2xl font-black text-rose-600 mt-2">€{overdueRevenue.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
            <div className="mt-4 flex items-center gap-2">
              <span className="relative flex h-2 w-2">
                <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-rose-400 opacity-75"></span>
                <span className="relative inline-flex rounded-full h-2 w-2 bg-rose-500"></span>
              </span>
              <span className="text-[10px] font-extrabold text-rose-600 bg-rose-50 px-2 py-0.5 rounded uppercase tracking-wider">{t('dashboard.overdueInvoices', { count: overdueInvoices.length })}</span>
            </div>
          </div>

          {/* Card 4: Total Revenue (This Month) - Standing Out in Sleek Luxury Dark Slate */}
          <div className="bg-gradient-to-br from-slate-900 via-slate-800 to-indigo-950 p-6 rounded-[2rem] shadow-xl shadow-indigo-950/20 relative overflow-hidden group flex flex-col justify-between border border-slate-800 hover:-translate-y-1 hover:shadow-2xl hover:shadow-indigo-950/30 transition-all duration-300">
            ...
            <div>
              ...
              <p className="text-2xl font-black text-white mt-2">€{totalRevenue.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
            </div>
            <div className="mt-4 flex items-center gap-2">
              <span className="relative flex h-2 w-2">
              ...
              <span className="text-[10px] font-extrabold text-emerald-400 bg-emerald-950/50 px-2 py-0.5 rounded uppercase tracking-wider">{t('dashboard.finalizedInvoices', { count: currentMonthPaidInvoices.length })}</span>
            </div>
          </div>
        </div>

        {/* Main Content: Full-width Management Modules */}
        <div className="bg-white p-8 rounded-[2rem] border border-slate-100/80 shadow-md shadow-slate-100/30 h-auto w-full">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 w-full">
            ...
            {['Admin', 'Employee_Stocks'].includes(user?.role) && (
              <div onClick={() => navigate('/inventory')} className="p-6 bg-white border border-slate-100/80 rounded-2xl hover:bg-amber-50/20 hover:border-amber-100 hover:shadow-md hover:shadow-amber-500/5 transition-all duration-300 cursor-pointer group">
        ...
        {/* Footer Content: Revenue Analytics & Alerts Side-by-Side */}
            ...
            <div>
              ...
              <div className="flex items-end justify-between h-48 gap-4 px-2">
                {Object.entries(monthlyData).map(([month, val]) => {
                  ...
                  return (
                    <div key={month} className="w-full h-full flex flex-col items-center group relative justify-end">
                      <div className="w-full bg-slate-50/50 rounded-2xl h-40 flex items-end overflow-hidden border border-slate-100 group-hover:bg-slate-50 transition-colors duration-300">
                        <div 
                          className="w-full bg-gradient-to-t from-indigo-600 via-indigo-500 to-violet-500 rounded-t-xl transition-all duration-500 group-hover:from-indigo-500 group-hover:via-violet-400 group-hover:to-fuchsia-400 shadow-lg shadow-indigo-500/10" 
            ...
            <div>
              ...
              <div className="space-y-4 overflow-y-auto max-h-[250px] pr-1 custom-scrollbar">
                {overdueInvoices.length > 0 ? overdueInvoices.map(inv => (
                  <div key={inv._id} className="p-4 bg-rose-50/30 border border-rose-100/50 rounded-2xl group hover:bg-rose-50/70 hover:border-rose-200 transition-all duration-300">
                    <div className="flex justify-between items-start mb-2">
                      <span className="text-[10px] font-black text-rose-600 font-mono tracking-tighter bg-rose-50 px-2 py-0.5 rounded">{inv.documentNumber}</span>
                      <span className="text-[10px] font-black text-rose-400 uppercase tracking-widest">{t('dashboard.overdue')}</span>
                    </div>
                    <p className="text-xs font-black text-slate-800 truncate">{inv.customer?.name}</p>
                    <div className="flex justify-between items-center mt-3 pt-3 border-t border-rose-100/50">
                      <span className="text-sm font-black text-slate-900">€{(inv.remainingBalance || inv.totalAmount).toLocaleString(undefined, { minimumFractionDigits: 2 })}</span>
                      <button 
                        onClick={() => navigate('/sales')} 
                        className="text-[9px] font-black uppercase tracking-widest text-rose-600 hover:text-rose-800 bg-rose-50 hover:bg-rose-100 px-3 py-1.5 rounded-lg transition-all duration-200 hover:shadow-sm"
                      >
                        {t('dashboard.collect')}
                      </button>
                    </div>
