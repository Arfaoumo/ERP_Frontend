...
const PipelineList = () => {
  ...
  return <div className="min-h-screen bg-[#f8fafc] p-8">
          ...
          <div className="p-8 border-b flex flex-col md:flex-row justify-between items-center bg-white gap-4">
            ...
            <div className="flex items-center gap-4">
              <div className="relative">
                <input type="text" placeholder={t('pipeline.searchPipeline')} value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="pl-10 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-2xl text-sm focus:ring-2 focus:ring-blue-500/20 outline-none w-64 transition-all" />
              ...
              <button onClick={() => navigate('/sales/new')} className="bg-blue-600 text-white px-6 py-2.5 rounded-2xl text-sm font-black uppercase tracking-widest hover:bg-blue-700 transition-all shadow-lg shadow-blue-200 active:scale-95">
                {t('pipeline.newQuote')}
              </button>
            </div>
          </div>
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody>
                {loading ? <tr>
                  ...
                  </tr> : filteredSales.map(s => <tr key={s._id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors group">
                    ...
                    <td className="px-8 py-6 text-center">
                      <div className="flex flex-col justify-center items-center gap-2">
                        ...
                        {s.status === 'Pending' && s.documentType === 'Order' && <button onClick={() => setModalConfig({
                      open: true,
                      sale: s,
                      action: 'convert'
                    })} className="text-[10px] font-black text-blue-600 hover:text-blue-800 uppercase tracking-widest transition-all">
                        ...
                        {s.payments && s.payments.length > 0 && <button onClick={() => navigate(`/sales/history/${s._id}`)} className="text-[10px] font-black text-blue-600 hover:text-blue-800 uppercase tracking-widest transition-all">
                            {t('pipeline.paymentHistory')}
                          </button>}
                        <button onClick={() => window.open(`/sales/invoice/${s._id}`, '_blank')} className="text-[10px] font-black text-gray-600 hover:text-gray-800 uppercase tracking-widest flex items-center gap-1 transition-all" title="View PDF">
      ...
      sale: null,
      action: ''
    })} onConfirm={handleConfirmAction} title={t('pipeline.pipelineTransition')} message={modalConfig.action === 'cancel' ? t('pipeline.cancelQuoteMsg', {
      number: modalConfig.sale?.documentNumber
    }) : modalConfig.sale?.documentType === 'Order' ? t('pipeline.convertOrderMsg', {
      number: modalConfig.sale?.documentNumber
    }) : t('pipeline.convertQuoteMsg', {
      number: modalConfig.sale?.documentNumber
    })} confirmText={modalConfig.action === 'cancel' ? t('pipeline.cancelQuoteBtn') : t('common.confirm')} type={modalConfig.action === 'cancel' ? 'danger' : 'primary'} />
    </div>;
};
