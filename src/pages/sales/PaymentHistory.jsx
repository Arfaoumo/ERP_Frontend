...
const PaymentHistory = () => {
  ...
  useEffect(() => {
    const fetchSale = async () => {
      try {
        ...
        const {
          data
        } = await axios.get(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/sales/${id}`, config);
        setSale(data);
      } catch (error) {
  ...
  if (loading) {
    return <div className="min-h-screen bg-gradient-to-br from-slate-50 via-[#f8fafc] to-slate-100/80 flex items-center justify-center">
        <div className="inline-block w-10 h-10 border-4 border-indigo-600/20 border-t-indigo-600 rounded-full animate-spin"></div>
      </div>;
  }
  if (!sale) {
    return <div className="min-h-screen bg-gradient-to-br from-slate-50 via-[#f8fafc] to-slate-100/80 flex flex-col items-center justify-center p-8">
        ...
        <header className="flex flex-col md:flex-row justify-between items-center p-6 rounded-[2rem] border border-white/50 backdrop-blur-md bg-white/70 shadow-lg shadow-slate-100/40 transition-all duration-300 gap-6">
          <div className="flex items-center gap-4 w-full md:w-auto">
            ...
            <div>
              ...
              <p className="text-[10px] font-extrabold text-slate-400 uppercase tracking-[0.2em] mt-0.5">{t('paymentHistory.invoiceNumber', {
          ...
          <div className="flex items-center gap-6 md:gap-8 bg-white/50 p-4 rounded-2xl border border-white/50 w-full md:w-auto justify-between md:justify-end">
            <div className="text-right">
              ...
              <p className="text-xl font-black text-slate-800">€{(sale.totalWithTax || sale.totalAmount).toLocaleString(undefined, {
            ...
            <div className="text-right">
              ...
              <p className="text-xl font-black text-rose-600">€{(sale.remainingBalance !== undefined ? sale.remainingBalance : sale.totalWithTax || sale.totalAmount).toLocaleString(undefined, {
                minimumFractionDigits: 2,
                maximumFractionDigits: 2
              })}</p>
            </div>
        ...
        <div className="bg-white p-8 rounded-[2rem] border border-slate-100/80 shadow-md shadow-slate-100/30">
          ...
          <div className="relative max-w-3xl mx-auto">
            ...
            <div className="space-y-8">
              {sale.payments && sale.payments.length > 0 ? sale.payments.map((payment, index) => <div key={index} className={`relative flex items-center justify-between md:justify-normal group ${index % 2 === 0 ? 'md:flex-row-reverse' : ''}`}>
                  ...
                  <div className={`w-full md:w-[calc(50%-3rem)] pl-16 md:pl-0 ${index % 2 === 0 ? 'md:pl-12' : 'md:pr-12'}`}>
                    <div className="bg-white p-6 rounded-2xl border border-slate-100/80 shadow-sm hover:shadow-xl hover:shadow-indigo-500/5 hover:-translate-y-1 transition-all duration-300 relative overflow-hidden group-hover:border-indigo-100">
                      ...
                      <div className="flex justify-between items-start mb-4">
                        <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest bg-slate-50 px-2.5 py-1 rounded-md">{new Date(payment.date).toLocaleDateString(i18n.language === 'fr' ? 'fr-FR' : 'en-US', {
                        weekday: 'short',
                        year: 'numeric',
                        month: 'short',
                        day: 'numeric'
                      })}</span>
                        <span className="text-xl font-black text-slate-800">€{payment.amount.toLocaleString(undefined, {
                        minimumFractionDigits: 2,
                        maximumFractionDigits: 2
                      })}</span>
                      </div>
                      ...
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-full bg-slate-100 flex items-center justify-center text-slate-500">
                          {payment.paymentMethod === 'Cash' ? <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="6" width="20" height="12" rx="2"></rect><circle cx="12" cy="12" r="2"></circle><path d="M6 12h.01M18 12h.01"></path></svg> : payment.paymentMethod === 'Bank Transfer' ? <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="5" width="20" height="14" rx="2"></rect><line x1="2" y1="10" x2="22" y2="10"></line></svg> : payment.paymentMethod === 'Check' ? <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"></path><polyline points="14 2 14 8 20 8"></polyline><path d="M16 13H8"></path><path d="M16 17H8"></path><path d="M10 9H8"></path></svg> : <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"></circle><polyline points="12 6 12 12 16 14"></polyline></svg>}
                        </div>
                        <div>
                          <p className="text-xs font-bold text-slate-700 uppercase tracking-wide">{t(`sales.${payment.paymentMethod.replace(/\s+/g, '')}`, {
                ...
                </div>) : <div className="text-center py-20 bg-slate-50/50 rounded-3xl border border-dashed border-slate-200">
