...
const SaleList = () => {
  ...
  const openModal = (sale, action) => {
    setModalConfig({ open: true, sale, action });
  };

  const openPaymentModal = (sale) => {
    setActiveSale(sale);
    setPaymentForm({ amount: (sale.remainingBalance || sale.totalAmount).toFixed(2), paymentMethod: 'Cash' });
  ...
  const filteredSales = sales.filter(s => {
    ...
    // For Billing: Pending means Invoice is Unpaid/Partially Paid OR Delivery Note is waiting to be Invoiced
    const isPendingFinancial = (s.documentType === 'Invoice' && (s.paymentStatus === 'Pending' || s.paymentStatus === 'Partially Paid' || s.paymentStatus === 'Overdue')) || 
                               (s.documentType === 'DeliveryNote' && s.status === 'In Transit');
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] p-8">
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody>
                {loading ? (
                  ...
                  <tr key={s._id} className="border-b hover:bg-gray-50/80 transition-colors">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-center font-black">
                      {s.documentType === 'Invoice' && s.remainingBalance > 0 ? (
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-center">
                      <div className="flex flex-col items-center gap-2">
                        {/* Top Status Badge: Hidden for Unpaid Invoices */}
                        {!(s.documentType === 'Invoice' && s.paymentStatus !== 'Paid') && (
                          <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider border ${getStatusBadgeClass(s.status)}`}>
                            {t(`status.${s.status}`, { defaultValue: s.status })}
                          </span>
                        )}

                        {/* Bottom Payment Badge: Only for Invoices and not fully Paid */}
                        {s.documentType === 'Invoice' && s.paymentStatus !== 'Paid' && (
                          <div className="flex flex-col items-center gap-1">
                            <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold uppercase ${getStatusBadgeClass(s.paymentStatus)}`}>
                              {t(`status.${s.paymentStatus}`, { defaultValue: s.paymentStatus })}
                            </span>
                            {/* Check Verification Badge logic removed */}
                          </div>
                        )}
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-center">
                      <div className="flex flex-col justify-center items-center gap-2">
                        {s.status === 'Pending' && s.documentType === 'Quote' && (
                          <button onClick={() => openModal(s, 'convert')} className="text-[10px] font-black text-blue-600 hover:text-blue-800 uppercase tracking-widest transition-all">{t('sales.convertToOrder')}</button>
                        )}
                        {s.status === 'Pending' && s.documentType === 'Order' && (
                          <button onClick={() => openModal(s, 'convert')} className="text-[10px] font-black text-blue-600 hover:text-blue-800 uppercase tracking-widest transition-all">{t('sales.createDeliveryNote')}</button>
                        )}
                        {s.status === 'In Transit' && s.documentType === 'DeliveryNote' && (
                          <button onClick={() => openModal(s, 'convert')} className="text-[10px] font-black text-blue-600 hover:text-blue-800 uppercase tracking-widest transition-all">{t('sales.issueInvoice')}</button>
                        )}
                        {s.documentType === 'Invoice' && (s.paymentStatus === 'Pending' || s.paymentStatus === 'Partially Paid' || s.paymentStatus === 'Overdue') && (
                          <button onClick={() => openPaymentModal(s)} className="text-[10px] font-black text-blue-600 hover:text-blue-800 uppercase tracking-widest transition-all">{t('sales.recordPayment')}</button>
                        )}
                        ...
                        {(s.documentType === 'Invoice' || s.documentType === 'DeliveryNote' || s.documentType === 'Quote' || s.documentType === 'Order') && (
                          <button onClick={() => window.open(`/sales/invoice/${s._id}`, '_blank')} className="text-[10px] font-black text-gray-600 hover:text-gray-800 uppercase tracking-widest flex items-center gap-1 transition-all">
      ...
      <ConfirmationModal 
        ...
        title={t('sales.convertDocument')}
        message={
          modalConfig.sale?.documentType === 'Order'
            ? t('sales.convertOrderMsg', { number: modalConfig.sale?.documentNumber })
            : t('sales.convertMsg', { type: modalConfig.sale?.documentType, number: modalConfig.sale?.documentNumber })
        }
        confirmText={t('common.confirm')}
        type="primary"
      />

      {showPaymentModal && activeSale && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-xl border border-slate-200">
            ...
            <p className="text-xs text-slate-500 mb-4">{t('sales.invoiceBalance', { number: activeSale.documentNumber, balance: (activeSale.remainingBalance || activeSale.totalAmount).toFixed(2) })}</p>
            <form onSubmit={handlePaymentSubmit} className="space-y-4">
              <div>
                <label className="block text-xs font-bold text-slate-700 mb-1 uppercase tracking-wider">{t('sales.amountLabel')}</label>
                <input type="number" step="0.01" required value={paymentForm.amount} onChange={(e) => setPaymentForm({...paymentForm, amount: e.target.value})} className="w-full p-2 border border-slate-200 rounded focus:ring-2 focus:ring-blue-500 focus:outline-none" />
              </div>
              <div>
                <label className="block text-xs font-bold text-slate-700 mb-1 uppercase tracking-wider">{t('sales.paymentMethod')}</label>
                <select value={paymentForm.paymentMethod} onChange={(e) => setPaymentForm({...paymentForm, paymentMethod: e.target.value})} className="w-full p-2 border border-slate-200 rounded focus:ring-2 focus:ring-blue-500 focus:outline-none">
                  <option value="Cash">{t('sales.cash')}</option>
                  <option value="Check">{t('sales.check')}</option>
                </select>
              </div>
              {/* Check Status select removed */}
