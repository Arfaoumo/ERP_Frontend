...
function Reports() {
  ...
  const fetchReports = async () => {
    ...
    try {
      ...
      const {
        data
      } = await axios.get(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/reports`, config);
      setReports(data);
      setError(null);
    } catch (err) {
  ...
  if (loading) {
    return <div className="flex justify-center items-center h-full min-h-screen bg-[#f8fafc]">
        ...
        <div className="bg-white rounded-[2rem] shadow-sm border border-slate-200/60 overflow-hidden">
          ...
          <div className="p-8 border-b flex flex-col md:flex-row justify-between items-center bg-white gap-4">
            ...
            <div className="flex flex-wrap items-center gap-4">
              <button onClick={() => handleExport('sales')} className="flex items-center gap-2 px-5 py-2.5 bg-slate-50 hover:bg-slate-100 text-slate-700 border border-slate-200 rounded-xl text-xs font-black uppercase tracking-widest transition-all active:scale-95">
                <Download size={14} />
                {t('reports.exportSales')}
              </button>
              <button onClick={() => handleExport('purchases')} className="flex items-center gap-2 px-5 py-2.5 bg-blue-600 hover:bg-blue-700 text-white shadow-lg shadow-blue-200 rounded-xl text-xs font-black uppercase tracking-widest transition-all active:scale-95">
                <Download size={14} />
          ...
          <div className="p-8">
            <div className="flex justify-between items-center mb-8 border-b border-slate-100 pb-4">
              ...
              <button onClick={handleGenerateManual} disabled={generating} className={`flex items-center gap-2 px-4 py-2 text-[10px] font-black uppercase tracking-widest rounded-lg transition-all ${generating ? 'bg-slate-100 text-slate-400' : 'bg-slate-900 hover:bg-slate-800 text-white shadow-md shadow-slate-900/20 active:scale-95'}`}>
            ...
            {reports.length === 0 ? <div className="text-center py-20 bg-slate-50/50 rounded-2xl border border-slate-100">
              ...
              return <div key={report._id} className="p-6 bg-slate-50/50 rounded-2xl border border-slate-100 relative group hover:bg-white hover:border-blue-100 hover:shadow-xl hover:shadow-blue-500/5 transition-all duration-300 hover:-translate-y-1">
                      <div className="flex justify-between items-center mb-6">
                        <span className="text-sm font-black text-slate-800 uppercase tracking-widest">{report.month} {report.year}</span>
                        {isProfitable ? <span className="flex items-center gap-1 text-[10px] font-black uppercase tracking-widest text-emerald-600 bg-emerald-50 border border-emerald-100 px-2.5 py-1 rounded-full"><TrendingUp size={12} /> {t('reports.profit')}</span> : <span className="flex items-center gap-1 text-[10px] font-black uppercase tracking-widest text-rose-600 bg-rose-50 border border-rose-100 px-2.5 py-1 rounded-full"><TrendingDown size={12} /> {t('reports.deficit')}</span>}
                      </div>
                      ...
                      <div className="space-y-5">
                        ...
                        <div>
                          ...
                          <p className="text-xl font-black text-slate-900 flex items-center gap-1">
                            €{report.totalPurchases.toLocaleString(undefined, {
                        minimumFractionDigits: 2
                      })}
                          </p>
                        ...
                        <div className="pt-5 border-t border-slate-200">
                          ...
                          <p className={`text-2xl font-black tracking-tighter ${isProfitable ? 'text-blue-600' : 'text-rose-500'}`}>
                            {report.profit >= 0 ? '+' : ''}€{report.profit.toLocaleString(undefined, {
                        minimumFractionDigits: 2
                      })}
                          </p>
                        </div>
